...
export class SnippetsService {
    static async getAll(filters: SnippetFilters = {}): Promise<CodeSnippetSummary[]> {
        ...
        let summaries: CodeSnippetSummary[] = (data ?? []).map((row: any) => {
            const tags = (row.code_snippet_tags ?? []).map((cst: any) => cst.tags).filter(Boolean);
            const allLanguages: string[] = (row.code_blocks ?? []).map((b: any) => b.language);
        ...
        if (filters.language) {
            summaries = summaries.filter((s) => s.languages.includes(filters.language!));
        }
    ...
    static async update(id: string, input: UpdateSnippetInput): Promise<CodeSnippet> {
        const supabase = await createClient();
    ...
    static async getDistinctLanguages(): Promise<string[]> {
        const supabase = await createClient();
        const { data, error } = await supabase.from('code_blocks').select('language');
        if (error) throw new Error(error.message);
        return [...new Set((data ?? []).map((r: any) => r.language as string))].sort();
    }
}
